...
```{r}
setwd('/Users/mehmetyoruten/Library/CloudStorage/OneDrive-bwedu/Documents/School/Master/CPI Lab/Thesis/Experiment/behavioral analyses/data/eb/')

participantID = 'eb'

# Choose your data set
df <- read.csv("eb.csv", header = TRUE)
df <- df[,-1]
...
#filename = paste('figs/', participantID, '/', participantID, '_test_all_coefficients.png', sep = "")
...
filename = paste('figs/', participantID, '/', participantID, '_control_all_coefficients.png', sep = "")
ggsave(filename = filename, width = 10, height = 5, device='png', dpi=300)
```
...
# Predict - Test Trials
Marcel's suggestion on re building the model
```{r}
require(ROCR)
#use 70% of dataset as training set and 30% as test set
...
trained_model <- glm(correct ~ cutNo*expTime, data = df, family = "binomial")
#trained_model <- glm(correct ~ cutNo*expTime + intOrder + avgCrossCorr, data = train, family = "binomial")
# get probabilities of getting correct answers
pHat <- predict(trained_model, type = 'response', interval = 'confidence')
df["pHat"] <- pHat

correct_prediction = prediction(pHat, df$correct)
correct_performance = performance(correct_prediction,
                                     measure = "tpr", # true positive rate
                                     x.measure = "fpr") # false positive rate
jitter = position_jitter(width = 0, height = 0.035)

ggplot(df) +
  geom_point(aes(x=cutNo, y = pHat, color = factor(expCond)), alpha = 0.2) +
  geom_line(aes(x=cutNo, y = pHat, color = factor(expCond))) + 
  theme_bw() +
  ylab("Accuracy") +
  xlab("Cuit No") + 
  theme_classic() + 
...
```{r}
df_grpd = df %>% group_by(cutNo, expTime) %>%
                summarise(acc = mean(correct), 
                          sd = sd(correct),
                          sem = sd(correct)/sqrt(length(correct)),
                          .groups = 'drop')

jitter = position_jitter(width = 0, height = 0.01)

ggplot(data = df_grpd, aes(x=cutNo, y = acc, color = expTime)) +
  geom_line() + 
  geom_errorbar(aes(ymin=acc-sem,ymax=acc+sem),width=0.1) + 
  geom_point(data =df, aes(x=cutNo, y = pHat, color = expTime)) + 
  theme_bw() +
  ylab("Accuracy") +
  xlab("Cuit No") + 
  theme_classic() + 
...
```{r}
ggplot(df_grpd) +
  geom_point(aes(x=cutNo, y = pHat, color = factor(expCond)), position = jitter, alpha = 0.2) +
  geom_line(dataaes(x=cutNo, y = pHat, color = factor(expCond)), position = jitter, alpha = 0.2) +
  #geom_point(aes(x=cutNo, y = correct, color=factor(correct)), position = jitter) +
  ylim(0,1)
```

Illustrate Receiver Operating Characteristics
Model comparison using AUC
```{r}
plot(correct_performance, colorize = TRUE) #colors correspond to cutoffs
performance(correct_prediction, "auc")@y.values
```
```{r}
require(ROCR)
#use 70% of dataset as training set and 30% as test set
...
trained_model <- glm(correct ~ cutNo*expTime + intOrder + segSize, data = train, family = "binomial")
#trained_model <- glm(correct ~ cutNo*expTime + intOrder + avgCrossCorr, data = train, family = "binomial")
# get probabilities of getting correct answers
pHat <- predict(trained_model, test ,type = 'response', interval = 'confidence')
#pHat <- predict(trained_model ,type = 'response', interval = 'confidence')

correct_prediction = prediction(pHat, test$correct)
correct_performance = performance(correct_prediction,
                                     measure = "tpr", # true positive rate
                                     x.measure = "fpr") # false positive rate
jitter = position_jitter(width = 0, height = 0.035)
...
ggplot(test) +
  geom_point(aes(x=cutNo, y = pHat, color = factor(correct)), position = jitter, alpha = 0.2) +
  #geom_point(aes(x=cutNo, y = correct, color=factor(correct)), position = jitter) +
  ylim(0,1)
```
