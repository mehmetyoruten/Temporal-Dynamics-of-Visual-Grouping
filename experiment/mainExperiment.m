...
%%                      Start Instructions
%----------------------------------------------------------------------
% DrawInstructionsStory(window, xCenter, yCenter, allCoords, lineWidthPix, numDots, hexEdge, hexAreaX, hexAreaY, hexXpos, hexYpos, textColor)
...
%     load testSegments.mat blockLayouts;
    
    % If you do not want to use control condition:
%     trialLayout(:,3) = 0;
    ...
    [trialLayout, ~] = SetExperimentOffline(numTest, trialImages, trialId, gridSecs, noControl);
...
%                 [segments] = NormMinCut(numDots, numIntensities, intensityMap, intensityDots, posDots, r, sigma_I, sigma_X, nCutThr);
                ...
                [segments, posDots, intensityDots, intensityMap, shownImages, chosenImg] = ChooseImage(trialImages,shownImages, trialSegment, intensities);
            ...
            % Generate segments
%             [segments] = NormMinCut(numDots, numIntensities, intensityMap, intensityDots, posDots, r, sigma_I, sigma_X, nCutThr);
            segments = trialImages{trialId(TrialNo)};

            % Choose a segment
            [seg, segCenter, segLevel, segNcutVal, segStab] = ChooseSegment(segments, trialSegment);
        end
                
        % Rotate the selected segment if control should be presented
        if (trialControl == 1)
            [segments, segInt, seg, shownFoils, chosenFoilImg]= GenerateFoil(controlSegments,segments, trialSegment, shownFoils, numDots, overlapThr,leafOnly);
        ...
        % Draw Segment 
        [imageArray, delta_int, intDiff, intStd,intOrder] = DrawHexSegment(window, intensityMap, intensityDotsControl, seg, segSecs, hexEdge, hexXpos, hexYpos);        
        ...
        imagesSegs{trialId(TrialNo), 5} = imageArray;
        ...
        presentedSegs{trialId(TrialNo),5} = chosenImg;          % presented img id
        presentedSegs{trialId(TrialNo),6} = chosenFoilImg;          % control img 
        presentedSegs{trialId(TrialNo),7} = selectedSegIdx;     % presented segment id from the img cell

        presentedSegs{trialId(TrialNo),8} = intensityDots;      % 10 x 10 array of presented image
        if trialControl == 1
            presentedSegs{trialId(TrialNo),9} = segInt;             % 10 x 10 array of the intensity array of segment
        else
            presentedSegs{trialId(TrialNo),9} = intensityDots;
        end
        ...
        respMatrix(trialId(TrialNo),8) = chosenImg;                             % Save the shown image
        respMatrix(trialId(TrialNo),9) = chosenFoilImg;                         % Save the shown image
        respMatrix(trialId(TrialNo),10) = selectedSegIdx;                       % Save the shown image
